...
class ShaskiyaMojniController extends GetxController with StateMixin<List<ShaskiyaMojniForm>> {
  ...
  void onInit() {
    ...
    if (args != null) {
      formType = args['formType'] ?? 'shaskiya_mojni';
      formName = args['formName'] ?? 'Shaskiya Mojni';

      developer.log('Arguments received: formType=$formType, formName=$formName');
    } else {
      formType = 'shaskiya_mojni';
      formName = 'Shaskiya Mojni';
    }
  ...
  Future<void> fetchFormsByType() async {
    try {
      ...
      final response = await ApiService.get<ShaskiyaMojniFormResponse>(
        endpoint: getPreviewData(int.parse(userId), formType),
        fromJson: (json) => ShaskiyaMojniFormResponse.fromJson(json),
        includeToken: true,
      );
  ...
  List<ShaskiyaMojniForm> getFormsByStatus(String status) {
  ...
  // Get forms with co-owners
  List<ShaskiyaMojniForm> getFormsWithCoOwners() {
    return state?.where((form) => form.hasCoOwners).toList() ?? [];
  }

  int getCoOwnersCount() {
    return getFormsWithCoOwners().length;
  }
