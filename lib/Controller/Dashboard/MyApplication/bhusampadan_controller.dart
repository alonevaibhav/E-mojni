...
class BhusampadanController extends GetxController with StateMixin<List<BhusampadanForm>> {
  ...
  void onInit() {
    ...
    if (args != null) {
      formType = args['formType'] ?? 'bhusampadan_citizen';
      formName = args['formName'] ?? 'Bhusampadan Citizen';

      developer.log('Arguments received: formType=$formType, formName=$formName');
    } else {
      formType = 'bhusampadan_citizen';
      formName = 'Bhusampadan Citizen';
    }
  ...
  Future<void> fetchFormsByType() async {
    try {
      ...
      final response = await ApiService.get<BhusampadanFormResponse>(
        endpoint: getPreviewData(int.parse(userId), formType),
        fromJson: (json) => BhusampadanFormResponse.fromJson(json),
        includeToken: true,
      );
  ...
  List<BhusampadanForm> getFormsByStatus(String status) {
  ...
  // Get bhusampadan approved forms
  List<BhusampadanForm> getBhusampadanApprovedForms() {
    return state?.where((form) => form.isBhusampadanApproved).toList() ?? [];
  }

  int getBhusampadanApprovedCount() {
    return getBhusampadanApprovedForms().length;
  }
