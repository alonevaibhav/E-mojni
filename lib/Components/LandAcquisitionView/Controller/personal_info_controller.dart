...
class PersonalInfoController extends GetxController
  ...
  // Validate office address fields with isEmpty checks
  void _validateOfficeAddressFields(Map<String, String> addressData) {
    if (addressData['address']?.trim().isEmpty ?? true) {
      officeAddressValidationErrors['address'] = 'Office address is required';
    }
  ...
  // NEW: Validate government survey fields (basic validation)
  bool _validateGovernmentSurveyFields() {
    // Applicant name validation with isEmpty check
  ...
  bool _validateLandAcquisitionFields() {
    ...
    // Land Acquisition Officer validation with isEmpty check
    if (landAcquisitionOfficerController.text.trim().isEmpty) {
      return false;
    }

    // Land Acquisition Board validation with isEmpty check
    if (landAcquisitionBoardController.text.trim().isEmpty) {
      return false;
    }

    // Land Acquisition Details validation with isEmpty check
    if (landAcquisitionDetailsController.text.trim().isEmpty) {
      return false;
    }

    // Land Acquisition Order Number validation with isEmpty check
    if (landAcquisitionOrderNumberController.text.trim().isEmpty) {
      return false;
    }

    // Land Acquisition Order Date validation with isEmpty check
    if (landAcquisitionOrderDateController.text.trim().isEmpty) {
      return false;
    }
    ...
    // File upload validations with isEmpty check
    if (landAcquisitionOrderFiles.isEmpty) {
      return false;
    }
  ...
  String getFieldError(String field) {
    switch (field) {
      case 'government_survey':
        return _getGovernmentSurveyValidationError();
      case 'land_acquisition_details':
        return _getLandAcquisitionValidationError();
      default:
        return 'This field is required';
  ...
  String _getGovernmentSurveyValidationError() {
    if (applicantNameController.text.trim().isEmpty) {
  ...
  String _getLandAcquisitionValidationError() {
    ...
    // Land Acquisition Order Number validation with isEmpty check
    if (landAcquisitionOrderNumberController.text.trim().isEmpty) {
      return 'Land Acquisition Order Number is required';
    }
  ...
  // NEW: Get specific field error for individual fields
  String getSpecificFieldError(String fieldType) {
    switch (fieldType) {
      case 'applicantName':
        ...
        break;
      case 'landAcquisitionOfficer':
        if (landAcquisitionOfficerController.text.trim().isEmpty) {
          return 'Land Acquisition Officer name is required';
        }
        break;
      case 'landAcquisitionBoard':
        if (landAcquisitionBoardController.text.trim().isEmpty) {
          return 'Land Acquisition Board details are required';
        }
        break;
      case 'landAcquisitionDetails':
        if (landAcquisitionDetailsController.text.trim().isEmpty) {
          return 'Land acquisition details are required';
        }
        break;
      case 'landAcquisitionOrderNumber':
        if (landAcquisitionOrderNumberController.text.trim().isEmpty) {
          return 'Land Acquisition Order Number is required';
        }
        break;
      case 'landAcquisitionOrderDate':
        if (landAcquisitionOrderDateController.text.trim().isEmpty) {
          return 'Land Acquisition Order Date is required';
        }
        break;
      case 'officeAddress':
        ...
        break;
      case 'landAcquisitionOrderFiles':
        if (landAcquisitionOrderFiles.isEmpty) {
          return 'Land Acquisition Order document is required';
        }
        break;
      case 'landAcquisitionMapFiles':
        if (landAcquisitionMapFiles.isEmpty) {
          return 'Land Acquisition Simankan Map is required';
        }
        break;
