...
class CensusSixthController extends GetxController with StepValidationMixin, StepDataMixin {
  ...
  void updateCoownerAddress(int index, Map<String, String> addressData) {
    if (index < coownerEntries.length) {
      ...
      validationErrors
          .removeWhere((key, value) => key.startsWith('${index}_address'));
      validationErrors
          .removeWhere((key, value) => key.startsWith('${index}_pincode'));
      validationErrors
          .removeWhere((key, value) => key.startsWith('${index}_village'));
      validationErrors
          .removeWhere((key, value) => key.startsWith('${index}_postOffice'));
  ...
  void showAddressPopup(BuildContext context, int index) {
    ...
    final addressControllers =
        addressControllersEntry as Map<String, TextEditingController>;
    final currentAddress = currentAddressEntry as Map<String, String>;

    // Populate controllers with current data safely
    addressControllers['plotNoController']?.text =
        currentAddress['plotNo'] ?? '';
    addressControllers['addressController']?.text =
        currentAddress['address'] ?? '';
    addressControllers['mobileNumberController']?.text =
        currentAddress['mobileNumber'] ?? '';
    addressControllers['emailController']?.text = currentAddress['email'] ?? '';
    addressControllers['pincodeController']?.text =
        currentAddress['pincode'] ?? '';
    addressControllers['districtController']?.text =
        currentAddress['district'] ?? '';
    addressControllers['villageController']?.text =
        currentAddress['village'] ?? '';
    addressControllers['postOfficeController']?.text =
  ...
  bool validateAllFields() {
    ...
    for (int i = 0; i < coownerEntries.length; i++) {
      ...
      final name =
          (entry['nameController'] as TextEditingController?)?.text ?? '';
      final mobileNumber =
